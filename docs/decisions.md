...
| 46 | 2026-09-20 | Keyword research makes zero LLM calls. Clustering is search-result overlap (four shared results in the top ten), naming is the highest-volume keyword, intent comes from the results, and page mapping starts from what Search Console already ranks. |
| 47 | 2026-09-20 | Clusters are built hub-first, never by chaining neighbours: a keyword joins the group it overlaps with, so one cluster cannot swallow a whole site. |
...
| 62 | 2026-09-21 | Setup is a resumable wizard. Its first step creates the organisation; every later step edits through the ordinary endpoints, so there is no second copy of the setup logic. Finishing needs one tracked topic and is what starts keyword research, which is then seeded from the team's own topics instead of a bare brand name. |
| 63 | 2026-09-21 | Site suggestions (names from the site's Organization data, topics from its navigation) are applied only while setup is unfinished. Names merge in as suggestions and never over a list a person edited; topics arrive proposed, and one the team skipped stays skipped. After setup the weekly crawl leaves the setup alone. |
| 64 | 2026-09-21 | "Test my setup" and detection share one matcher (`detect.matches`), and the test runs on unsaved edits against stored answers or pasted text. The preview is the count the reports will show, not an approximation of it. |
| 65 | 2026-09-21 | Every path that makes a topic active gives it prompts (`promptsForTopic`). Approving a proposed topic used to create none, so nothing measured it. |
| 66 | 2026-09-21 | An OAuth consent returns to a dashboard page named by key from a fixed list and carried in the signed state, never by URL, so the callback cannot become an open redirect. Changing the brand's domain re-crawls the site. |
| 67 | 2026-09-22 | Hosting: the api and worker run on Fly.io in Sydney from one image (`migrate` is the release command, so a failed migration stops the release), Postgres on Neon in Sydney, the dashboard on Vercel. Customer data stays in Australia, as BigQuery's `australia-southeast1` already does. |
| 68 | 2026-09-22 | `db.Open` refuses a database user that is a superuser or has BYPASSRLS, in every role and not only in tests. Managed Postgres hands out such an admin user by default, and connecting as it would show every tenant's rows with nothing failing. |
| 69 | 2026-09-22 | `VELLATRY_ENV=production` refuses development-only settings: header sign-in, a missing secret key, localhost URLs, and Google connections without BigQuery (raw facts would live in memory). It lists every problem at once, and a test holds `fly.toml` to the same checks. |
| 70 | 2026-09-22 | Continuous deploy runs only after `ci` passes on `main`, deploys the exact commit CI tested, and stays off until a Fly token is added to the repository. |
| 71 | 2026-09-22 | Sign-in is Clerk on the client only: no middleware and no server-side auth, because the dashboard does no server-side data fetching (#56) and the api verifies every token. The token comes from Clerk per request and is never written to browser storage. |
| 72 | 2026-09-22 | Clerk is mounted only when a publishable key is set. Without one, its SDK provisions a temporary application of its own. Local development instead keeps the api's header sign-in, which production refuses (#69). |
| 73 | 2026-09-22 | The landing page lives at `/` and is the only indexable page; the app moved to `/today` and stays noindex. Every claim on it describes shipped behaviour, and its illustrations use "Your brand" and "Competitor A" with an "Illustration" caption: no real brands, customers, statistics or prices. |
| 74 | 2026-09-22 | The design language is a project skill (`.claude/skills/vellatry-design`), not a component library: tokens, type, components and copy rules in one place that every session loads before designing. The serif display face (Instrument Serif, self-hosted by `next/font`) is for marketing headlines only. |
| 75 | 2026-09-22 | The design moved to "paper and highlighter" (supersedes the type choice in #74). One family, Archivo, stretched wide and heavy for marketing headlines and at normal width in the app. Warm paper instead of flat white, lime for your mentions, coral for a competitor's, and dark bands. The serif-italic headline read as a stock AI landing page. The mention colours are shared by the app and the landing page, so the product tour on the landing page shows the real product's colours. |
| 76 | 2026-09-22 | Motion is CSS and progressive: every animation runs from a hidden or partial state to the element's ordinary style, and scroll reveals use `animation-timeline: view()` inside `@supports`. Reduced motion (which Windows' "Animation effects" switch sets) removes movement, not change: rises become fades, loops and autoplay stop, and colour-ins and wipes stay. |
| 77 | 2026-09-22 | On Fly the worker reaches BigQuery with a service-account key in a secret (`BIGQUERY_CREDENTIALS_JSON`, raw or base64), loaded with the type-restricted `option.WithAuthCredentialsJSON(option.ServiceAccount, ...)`, so a credential of another type is refused rather than trusted. The project comes from the key. Keyless federation from Fly's OIDC tokens is the better end state once there is a reason to set it up. |
